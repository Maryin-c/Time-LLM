...
class SlidingWindowMask:
    def __init__(self, seq_len, window_size, device):
...
class TSAttention(nn.Module):
    '''Time-aware Sliding Window Attention'''
    def __init__(self, mask_flag=True, window_size=10, scale=None, attention_dropout=0.1, output_attention=False):
        ...
        self.delta_param = nn.Parameter(torch.tensor(0.0))

    def forward(self, queries, keys, values, attn_mask=None, time_info=None):
        ...
        if self.mask_flag:
            if attn_mask is None:
                sw_mask = SlidingWindowMask(L, self.window_size, device=queries.device).mask
                attn_mask = sw_mask.unsqueeze(0).unsqueeze(0)  # [1, 1, L, L]
